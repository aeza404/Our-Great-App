...
function renderRecipeSelection() {
  ...
  if (recipes.length === 0) {
    container.innerHTML = '<p style="color:#94a3b8;font-size:1.3rem;text-align:center;padding:1rem;">No recipes found. Add recipes in the Recipes tab first.</p>';
    return;
  }
...
function generateSmartShoppingList() {
  if (selectedRecipeIndices.size === 0) {
    showToast('Please select at least one recipe first');
    return;
  }
  ...
  selectedRecipes.forEach(recipe => {
    (recipe.ingredients || []).forEach(ing => {
      ...
      // Add quantity (handle both numbers and empty strings)
  ...
  Object.values(neededIngredients).forEach(ing => {
    ...
    // If we don't have enough, add to shopping list
    if (inStock < needed) {
  ...
  if (shoppingList.length === 0) {
    showToast('You already have everything you need!');
    return;
  }
  ...
  showToast(`Added ${shoppingList.length} items to your shopping list!`);
  render();

  // Save to localStorage for persistence
  localStorage.setItem('kk_shoppingList', JSON.stringify(state.items));
}

// Set up event listeners for smart shopping
document.addEventListener('DOMContentLoaded', () => {
  renderRecipeSelection();
  
  const generateBtn = document.getElementById('generateSmartListBtn');
  if (generateBtn) {
    generateBtn.addEventListener('click', generateSmartShoppingList);
  }
});